...
from bigtableio_read_2 import ReadFromBigtable
...
class BigtableReadTest(unittest.TestCase):
  ...
  def test_bigtable_read(self):
    ...
    logging.info('Reading table "{}"...'.format(TABLE_ID))

    p = beam.Pipeline(options=p_options)

    _ = (p | 'Read Test' >> ReadFromBigtable(PROJECT_ID, INSTANCE_ID, TABLE_ID))

    self.result = p.run()
    self.result.wait_until_finish()
    assert self.result.state == PipelineState.DONE

    query_result = self.result.metrics().query(MetricsFilter().with_name('Rows Read'))
